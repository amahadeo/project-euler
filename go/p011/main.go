...
func spaceRight(x, y int) bool {
	return x+(consecutive-1) < len(grid[y])
}

func spaceDown(y int) bool {
	return y+(consecutive-1) < len(grid)
}

func spaceUp(y int) bool {
	return y >= (consecutive - 1)
}

func checkProduct(factors []int) {
	product := 1
